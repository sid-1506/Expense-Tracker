...
export default function App() {
  ...
  // IMPORTANT: These values were previously stored in global localStorage keys,
  // which leaked between different logged-in Supabase users on the same browser.
  // We now scope them by the authenticated user id.
  ...
  // ── Load per-user local state (top-ups + monthly budget) ───────────────────
  useEffect(() => {
    if (!user) {
      setTopUps({});
    ...
    try {
      const tu = JSON.parse(localStorage.getItem(`et_tu_${user.id}`) || "{}");
      setTopUps(tu && typeof tu === "object" ? tu : {});
    } catch {
      setTopUps({});
    }

    try {
      const bgRaw = JSON.parse(localStorage.getItem(`et_bg_${user.id}`) || "10000");
      const bg = typeof bgRaw === "number" ? bgRaw : parseFloat(bgRaw);
      setMonthlyBudget(Number.isFinite(bg) ? bg : 10000);
    } catch {
      setMonthlyBudget(10000);
    }
  }, [user]);
  ...
  useEffect(() => {
    if (!user) return;
    localStorage.setItem(`et_tu_${user.id}`, JSON.stringify(topUps));
    localStorage.setItem(`et_bg_${user.id}`, JSON.stringify(monthlyBudget));
  }, [user, topUps, monthlyBudget]);
  ...
  // ── Add / Update Expense ────────────────────────────────────────────────────
  const addExpenseToDB = async (expense, userId) => {
    try {
      const { data, error } = await supabase
        .from("Expenses")
        .insert([{
  ...
  const handleAddOrUpdate = async (e) => {
    ...
    if (isEditing) {
      ...
      const ok = await addExpenseToDB(newExpense, userId);
      if (ok) addToast("Expense added");
    }
